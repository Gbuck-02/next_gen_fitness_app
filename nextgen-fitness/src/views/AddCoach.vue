<template>
    <div class="coachCode-container">
      ...
      <label for="coachCode">Enter Coach Code:</label>
      <input type="text" id="coachCode" v-model="coachCode" placeholder="Enter your coach's code" />
    ...
    methods: {
      ...
      submitCoachCode() {
        ...
        fetch(`http://localhost:3000/api/addCoachInvite`, {
          ...
          body: JSON.stringify({ username: this.username, coachCode: this.coachCode })
        })
        .then(response => response.json())
        .then(data => {
          if (data.success) {
            alert("Coach request sent!");
            this.home(); // Redirects immediately
          } else {
